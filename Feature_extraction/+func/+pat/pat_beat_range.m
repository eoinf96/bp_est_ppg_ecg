function PAT = pat_beat_range(PPG, ECG, PAT_min_value, PAT_max_value, plot_flag, fig_h,configs)
% PAT_beat_range computes beat by beat pulse arrival time (PAT) from an
...
%        PPG: PPG time series
%        PAT_min_flag: min search window (if value = 0 then search window starts at the beginning of each ECG R-peak)
%        PAT_max_flag: max search window
